...
- `NEXORA_SYMBOLS` controls the read-only market watchlist and candle ingestion. `NEXORA_TRADE_SYMBOLS` controls which enabled symbols receive automated paper scans; production defaults to `XAUUSD` so the four-symbol watchlist remains live without running unnecessary entry scans for the supporting FX pairs.
...
- `GET /api/dashboard`, `/bot/status`, `/api/market`, `/api/markets`, `/api/market/overview`, `/api/market/candles?timeframe=M15|M30|H1|H4`, `/api/candles?symbol=XAUUSD&timeframe=M15`, `/api/mtf/latest`, `/api/news`, `/api/positions`, `/api/orders`, `/api/trades?limit=25&cursor=...`, `/api/stats`, `/api/audit`, `/api/scan/latest`, `/api/lastscan`.

...
The dashboard is read-only until a local operator token is configured. Set `NEXORA_CONTROL_TOKEN` in `.env` to a newly generated random value of at least 32 characters, restart the service, then enter it in the dashboard's local control field. The page keeps the token in memory only and sends it only to this loopback API; it is not stored in browser storage or logs. Never paste the token into chat or commit `.env`.
